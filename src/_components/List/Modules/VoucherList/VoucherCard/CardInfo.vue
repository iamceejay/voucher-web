<template>
  <div
    ...
    @click="onClickHeader()"
  >
    <div class="card-header p-3 2xl:p-5">
    ...
    <div class="card-content">
      <div class="w-full flex flex-col break-words relative">
        ...
        >
          <svg
            ...
            :class="[
              {
                'text-peach': AUTH_USER_VOUCHER_WISHLIST.find(
                  (_wishlist) => _wishlist.voucher_id == voucher.id
                ),
              },
            ]"
          >
      ...
      <div
        class="flex flex-col px-3 pt-3 2xl:px-5 2xl:pt-5 break-words card-description h-20"
      >
        <div class="text-sm leading-5 border-b border-input-border h-full">
          {{
            (voucher.note || voucher.description).length > 80
              ? (voucher.note || voucher.description).slice(0, 80) + '...'
      ...
      <div class="flex flex-row card-footer">
        ...
        <div class="w-1/2 flex flex-col p-3 2xl:p-5 h-28 md:h-32">
          ...
          <template v-else>
            ...
            <span v-if="!voucher.valid_day" class="text-2xs opacity-50">
...
export default {
  ...
  methods: {
    ...
    async onClickWishlist(voucher) {
      ...
      const validate = this.AUTH_USER_VOUCHER_WISHLIST.find(
        (_wishlist) => _wishlist.voucher_id == voucher.id
      );
      if (validate) {
        payload = {
        ...
        try {
          ...
          await localStorage.setItem(
            '_userWishlist',
            JSON.stringify(this.AUTH_USER_VOUCHER_WISHLIST)
          );
        ...
        try {
          ...
          const { user_voucher_wishlist } = await this.$store.dispatch(
          ...
          await localStorage.setItem(
            '_userWishlist',
            JSON.stringify(user_voucher_wishlist)
          );
          await this.$store.commit(
            'SET_AUTH_USER_VOUCHER_WISHLIST',
